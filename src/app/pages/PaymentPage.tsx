...
type PaymentMethod = "wechat" | "alipay" | "service";
...
export function PaymentPage() {
  ...
  function submit(event: FormEvent) {
    ...
    const label = method === "wechat" ? "Nạp WeChat Pay" : method === "alipay" ? "Nạp Alipay" : "Thanh toán dịch vụ tại Trung Quốc";
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_420px]">
          <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-[#ece2e0]">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-3">
              {[["wechat", "Nạp WeChat Pay"], ["alipay", "Nạp Alipay"], ["service", "Thanh toán dịch vụ"]].map(([value, label]) => <button key={value} onClick={() => setMethod(value as PaymentMethod)} className={`rounded-2xl p-4 text-left font-bold ${method === value ? "bg-[#b7131a] text-white" : "bg-[#f8f3f2] text-[#5b403d]"}`}>{label}</button>)}
            </div>
            <form onSubmit={submit} className="mt-6 grid gap-4 md:grid-cols-2">
              ...
              <button className="rounded-xl bg-[#b7131a] px-5 py-3 font-bold text-white">Gửi yêu cầu</button>
            </form>
